...
# Example personalization [SIGN-700]: when serving a word's synsets, rewrite
# example sentences to use the word being viewed instead of the synonym the
# example was authored with ("wore a goofy hat" -> "wore a zany hat" when
# viewing "zany"). English only. Deliberately conservative: an example is
# returned unchanged unless every guard in personalize_example passes.
